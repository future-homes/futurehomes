...
export default function SearchPage() {
  ...
  // Check if nearMe parameter exists and set initial sort to 'distance'
  const isNearMeMode = searchParams.get('nearMe') === 'true';
  const [sortBy, setSortBy] = useState(isNearMeMode ? 'distance' : 'newest');

  useEffect(() => {
    fetchProperties();
    
    // If nearMe mode, request location immediately and set distance to 20km
    if (isNearMeMode) {
      getUserLocation(true);
      setFilters(prev => ({ ...prev, distance: 20 }));
    } else {
  ...
  const getUserLocation = (showPrompt: boolean = false) => {
    if (navigator.geolocation && !locationRequested) {
      setLocationRequested(true);
      
      if (showPrompt) {
        console.log('📍 Getting your location for nearby properties...');
      }
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (showPrompt) {
            console.log('✅ Location detected! Showing nearby properties');
          }
        },
        (error) => {
          console.log('Location access denied:', error);
          if (showPrompt) {
            alert('📍 Please enable location access to see nearby properties');
          }
        }
      );
    }
  ...
  const applyFiltersAndSort = () => {
    ...
    if (filters.minArea) filtered = filtered.filter(p => p.area >= parseInt(filters.minArea));
    if (filters.maxArea) filtered = filtered.filter(p => p.area <= parseInt(filters.maxArea));
    if (filters.amenities.length > 0) {
      filtered = filtered.filter(p => 
        filters.amenities.every(amenity => p.amenities.includes(amenity))
      );
    }
    if (filters.availability) filtered = filtered.filter(p => p.availability === filters.availability);
    ...
    switch (sortBy) {
      ...
      case 'area-large': filtered.sort((a, b) => b.area - a.area); break;
      case 'area-small': filtered.sort((a, b) => a.area - b.area); break;
  ...
  const resetFilters = () => {
    setFilters({
      search: '', city: '', propertyType: '', minPrice: '', maxPrice: '',
      bedrooms: '', bathrooms: '', furnishing: '', minArea: '', maxArea: '',
      tenant: '', amenities: [], distance: 50, availability: '',
    });
  };
  ...
  const FilterContent = () => (
    <div className="space-y-6">
      ...
      {/* Price Range */}
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-2">Price Range (₹/month)</label>
      ...
      {userLocation && (
        <div>
          ...
          <input type="range" min="5" max="50" step="5" value={filters.distance}
            onChange={(e) => setFilters({ ...filters, distance: parseInt(e.target.value) })}
            className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600"/>
          <div className="flex justify-between text-xs text-gray-500 mt-1">
            <span>5 km</span><span>50 km (All)</span>
          </div>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 pb-24 md:pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Near Me Banner */}
        {isNearMeMode && (
          <div className="mb-6 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl p-4 sm:p-6 text-white shadow-lg">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-lg mb-1"> Showing Properties Near You</h3>
                <p className="text-sm text-blue-100">
                  {userLocation 
                    ? `Sorted by distance • Within ${filters.distance}km radius`
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            {isNearMeMode ? 'Properties Near You' : 'Search Properties'}
          </h1>
        ...
        <div className="lg:hidden fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white rounded-t-3xl w-full">
            <div className="border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowMobileSort(false)} className="p-2 hover:bg-gray-100 rounded-lg">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ...
            <div className="p-4">
              {[
                ...
                <button key={option.value}
                  onClick={() => { setSortBy(option.value); setShowMobileSort(false); }}
